...
class AutoFishing:
    ...
    def _monitor_area(self):
        ...
        while not self.stop_flag:
            ...
            try:
                ...
                #
                # # 显示当前面积和状态
                # status = "等待" if not self.fishing else "钓鱼中"
                # cv2.putText(display_frame, f"Area: {total_area:.2f} | Status: {status}",
                #             (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                #
                # # 显示实时画面
                # cv2.imshow('Fishing Monitor', display_frame)
                cv2.waitKey(1)
    ...
    def _wait_for_fishing_start(self):
        ...
        while not self.stop_flag and not self.reset_flag and time.time() - start_time < 20:  # 最多等待20秒
            ...
            # 检查是否有面积出现（鱼上钩）
            with self.lock:
                if self.current_area > 200:  # 设置一个阈值，表示鱼已上钩
                    self.initial_area = self.current_area
                    print(f"鱼已上钩！初始面积: {self.initial_area}")
                    return True
    ...
    def _fishing_process(self):
        ...
        while not self.stop_flag and not self.reset_flag and not self.fish_caught:
            ...
            if current < 5 and not self.reeling:  # 设置一个接近0的阈值
                print("面积接近0，拉线阶段结束，进入收线阶段")
            ...
            if not self.reeling:
                ...
                # 只有当面积减少超过20时才认为是有效的变化
                if area_change > 20:  # A键有效
                    print(f"鱼向右游，按A键有效，面积减少: {area_change}")
                    ...
                    # 只有当面积减少超过20时才认为是有效的变化
                    if area_change > 20:  # D键有效
                        print(f"鱼向左游，按D键有效，面积减少: {area_change}")
                        # 如果按键后进入了收线阶段，则不再继续执行后续逻辑
            ...
            if self.reeling:
                ...
                # 快速右键点击
                for _ in range(10):  # 每次循环点击10次，然后检查状态
                    if self.stop_flag or self.reset_flag:
                        break
                ...
                with self.lock:
                    ...
                    if reeling_duration > 2.0 and self.current_area < 50 and self.reeling_clicks > 20:
                        print(f"收线完成，钓鱼结束 (用时: {reeling_duration:.1f}秒, 点击: {self.reeling_clicks}次)")
                    ...
                    # 如果面积接近初始面积，需要继续拉鱼
                    if self.current_area > self.initial_area * 0.95:
                        print(f"面积增大至 {self.current_area}，需要继续拉鱼")
                        self.reeling = False
                        # 重置收线状态
                        delattr(self, 'reeling_start_time')
                        delattr(self, 'reeling_clicks')
    ...
    def _press_key_until_area_decreases(self, key):
        ...
        for _ in range(max_attempts):
            ...
            # 只有在未进入收线阶段时才执行这个检查
            if current < 30 and not self.reeling:  # 设置一个接近0的阈值
                print("面积接近0，拉线阶段结束，进入收线阶段")
                self.reeling = True
            ...
            # 检查面积是否减少
            if last_check_area - current > 20:  # 面积有效减少
                print(f"面积有效减少: {last_check_area - current}")
        ...
        return start_area - final_area > 30  # 返回是否有效减少过面积
